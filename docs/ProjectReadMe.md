...
**Location:** `technical-assignment/src/number_generator.py`
...
**Location:** `technical-assignment/src/number-generator-script.py`
...
#### Helper Functions:
- `_load_data()`: Loads the MNIST data from a directory containing training images and labels. The MNIST data is present inside the `technical-assignment/resources` directory. Function `generate_numbers_sequence()` internally calls the `_load_data()` and saves the loaded images and labels in a global variable during the initial call.
...
* **Test case files:** `technical-assignment/tests/test_number-generator-script.py`, `technical-assignment/tests/test_number_generator.py`
* **Number of test cases:** 21 Test cases
* **Type of test cases:** Unit-tests and Functional tests
* **Run test cases:**
    Go inside the `technical-assignment` folder and executing the following command.
     ```commandline
     python -m pytest
     ```
